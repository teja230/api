...
const JiraIntegration = ({ enterpriseConfig }) => {
  ...
  const handleDisconnect = async () => {
    try {
      const response = await fetch(`/api/jira/disconnect?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Accept': 'application/json'
  ...
  const handleSettingsSave = async () => {
    try {
      const response = await fetch(`/api/jira/settings?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const handleCreateIssue = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/jira/issues?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
