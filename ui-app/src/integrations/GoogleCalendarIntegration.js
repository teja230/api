...
const GoogleCalendarIntegration = ({ enterpriseConfig }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (calendarsResponse.status === 401 || eventsResponse.status === 401) {
        window.location.href = '/login';
        return;
      }
  ...
  const handleDisconnect = async () => {
    try {
      const response = await fetch(`/api/google/disconnect?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Accept': 'application/json'
  ...
  const handleSettingsSave = async () => {
    try {
      const response = await fetch(`/api/google/settings?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
