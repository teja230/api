...
const GitHubIntegration = ({ enterpriseConfig }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (reposResponse.status === 401 || teamsResponse.status === 401) {
  ...
  const handleDisconnect = async () => {
    try {
      const response = await fetch(`/api/github/disconnect?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Accept': 'application/json'
  ...
  const handleSettingsSave = async () => {
    try {
      const response = await fetch(`/api/github/settings?enterpriseId=${enterpriseConfig?.enterpriseId}`, {
        method: 'POST',
        credentials: 'include',
        headers: {
